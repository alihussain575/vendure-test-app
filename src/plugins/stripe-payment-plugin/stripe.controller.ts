...
export class StripeController {
	...
	async webhook(
		...
		const { metadata: { channelToken, orderCode, orderId } = {} } =
			paymentIntent;
		...
		await this.connection.withTransaction(outerCtx, async (ctx) => {
			...
			try {
				...
				Logger.error(
					`${signatureErrorMessage} ${signature}: ${(e as Error)?.message}`,
					loggerCtx
				);
			...
			if (event.type === "payment_intent.payment_failed") {
				...
				Logger.warn(
					`Payment for order ${orderCode} failed: ${message}`,
					loggerCtx
				);
			...
			if (event.type !== "payment_intent.succeeded") {
				...
				Logger.info(
					`Received ${event.type} status update for order ${orderCode}`,
					loggerCtx
				);
				return;
			}
			...
			// const stripePaymentMethod = await this.retrivePaymentMethod(ctx, order, paymentIntent.payment_method);
			// console.log(paymentIntent);
			const stripePaymentMethod = await this.retrivePaymentMethod(
				ctx,
				order,
				paymentIntent.payment_method as string
			);
			const card = stripePaymentMethod.card as Stripe.PaymentMethod.Card;
	...
	private async retrivePaymentMethod(
		ctx: RequestContext,
		order: Order,
		stripPaymentMethodId: string
	) {
		const stripeClient = await this.stripeService.getStripeClient(ctx, order);

		const stripePaymentMethod = await stripeClient.paymentMethods.retrieve(
			stripPaymentMethodId
		);
