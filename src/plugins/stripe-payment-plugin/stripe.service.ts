...
export class StripeService {
	...
	async createPaymentIntent(
		...
		if (this.options.storeCustomersInStripe) {
	...
	async getStripeClient(
		...
		const isEligible = eligiblePaymentMethods.some(
			(pm) => pm.code === stripePaymentMethod.code
		);
		if (!isEligible) {
			throw new UserInputError(
				`Stripe payment method is not eligible for order ${order.code}`
			);
		}
		const apiKey = this.findOrThrowArgValue(
			stripePaymentMethod.handler.args,
			"apiKey"
		);
		const webhookSecret = this.findOrThrowArgValue(
			stripePaymentMethod.handler.args,
			"webhookSecret"
		);
	...
	private async getStripeCustomerId(
		...
		if (stripeCustomers.data.length > 0) {
			...
			Logger.info(
				`Created Stripe Customer record for customerId ${customer.id}`,
				loggerCtx
			);
		}
